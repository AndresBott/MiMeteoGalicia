...
//##########################################################################################################################################################
	...
	/**
	 * Escapes a sting to avoid sql injections
	 * @param string $string
	 * @return string
	 */
...
//##########################################################################################################################################################
	...
	/**
	 * Escapes an key value array using self::escapeString();
	 * @param array $array
	 * @return array
	 *
	 */
	static public function escapeArray($array=[]){
		...
		foreach ($array as $key => $val){
			...
			if(is_array($val)){
				$tmp[self::escapeString($key)] = self::escapeArray($val);
			}else{
				$tmp[self::escapeString($key)] = self::escapeString($val);
			}
...
//##########################################################################################################################################################
	 ...
	 */
        ...
        //              http://stackoverflow.com/questions/9504608/request-string-without-get-arguments-in-php
                ...
                $url = "$sheme://$_SERVER[HTTP_HOST]$uri_parts[0]";   
                $extensions = ["php","html"];
                foreach($extensions as  $extension){
                        if($extension == self::stringGetFromRight($url,strlen($extension))){
                                return dirname($url);
                        }
                }
	...
	/**
	 * check if we can open a connection to an url;
	 * @param $url to checl
	 * @param int $port por to check 80|| 443
	 * @return bool
	 */
	static function urlReachable($url){
